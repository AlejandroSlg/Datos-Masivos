/*Muestras de datos correspondientes a dos de las temporadas de Maria*/
val muestra0 = List(10,5,20,20,4, 5, 2, 25, 1)
val muestra1 = List(3,4,21,36,10,28,35,5,24,42)

/*Definición de la función*/
def breakingRecords(scores:List[Int]): Unit =//Se define la variable de la funcion y el tipo de dato del parametro de entrada, la cual es en esta caso una lista
{
    var min = scores(0) //*"min" tomara el primer elemento de la lista a analizar
    var max = scores(0) //*"max" tomara el primer elemento de la lista a analizar
    var cont_min = 0
    var cont_max = 0

    /*Declaración de ciclo for para comparar los scores de manera iterativa*/
     for (i <- scores) //Para cada valor de i de la variable "Num" de la funcion "breakingRecords"
    {
    ...
    println("Resultados:")//Comentarios shidos
    println ("Maximo:"+cont_max, " Minimo:"+cont_min)//Impresiones shidas
}
